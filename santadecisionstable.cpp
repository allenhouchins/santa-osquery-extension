...
osquery::QueryData decisionTablesGenerate(osquery::QueryContext& request,
  ...
  for (const auto& entry : log_entries) {
    ...
    r["path"] = entry.application;
    r["shasum"] = entry.sha256;
...
osquery::TableRows SantaAllowedDecisionsTablePlugin::generate(
  ...
  // Using the same pattern as in santarulestable.cpp
  // Convert the QueryData to TableRows format expected by osquery
  
  // This empty result will trigger osquery's fallback mechanism
