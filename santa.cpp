...
bool collectSantaRules(RuleEntries& response) {
  ...
  try {
    fs::remove(kTemporaryDatabasePath);
  } catch (const std::exception& e) {
    VLOG(1) << "Failed to remove temporary database file: " << e.what();
  }
...
RuleEntry::State getStateFromRuleName(const char* name) {
  ...
  if (state_name == "blacklist" || state_name == "denylist") {
    return RuleEntry::State::Blacklist;
  } else if (state_name == "whitelist" || state_name == "allowlist") {
